...
var Tank = {
    ...
    new: func (name, index, typical_level=nil) {
        ...
        # aerial refuelling
        tank_property.initNode("selected", 0, "BOOL");
    ...
    prepare_subtract_fuel_flow: func (flow, dt) {
        var removed_gal_us = me.test_subtract_fuel_flow(flow, dt);
    ...
    prepare_add_fuel_flow: func (flow, dt) {
        var added_gal_us = me.test_add_fuel_flow(flow, dt);
...
#        debug.dump("xxx", flow, me.total_flow_sinks);
...
#        debug.dump("yyy", flow, me.total_flow_sources, me.transferable_flow);
...
var AbstractPump = {
    ...
    transfer_fuel: func (dt) {
        ...
        if (current_flow > 0.0) {
            ...
            if (abs(flow - actual_flow) > 0.0000001) {
                debug.dump(abs(flow - actual_flow), me.get_name(), current_flow, available_flow, actual_flow, flow);
            }
...
var AutoPump = {
    ...
    # A pump which will always maximize the flow. Unless you have some
    # other pump that will cause fuel to flow, you need to attach an
    # AutoPump to an EngineConsumer to make it demand fuel, since an
    # EngineConsumer is by itself passive. An AutoPump is especially
    # useful (and needed) to transfer fuel from a feeder tank to an engine.
    #
...
var EngineConsumer = {
    ...
    prepare_add_fuel_flow: func (flow, dt) {
        me.consumed_gal_us = me.test_add_fuel_flow(flow, dt);
        return me.consumed_gal_us;
    }

};

var TankEngineConsumer = {

    # An EngineConsumer that actually fills a tank with the consumed
    # amount of fuel.
    #
    # This component is useful for JSBSim aircraft where you need to
    # have a very small feeder tank for the JSBSim engine.

    new: func (name, engine, index) {
        # Create a new instance of TankEngineConsumer.
        #
        # engine: A function f(flow) that is given a certain amount of
        #         flow (gal/s) to be used to let the engine provide the
        #         desired thrust. It must return a value that represents
        ...
        #         need a higher flow to provide the desired thrust, then the
        #         engines have no choice but to provide less thrust than desired.
        # index:  The index of the tank

        var tank_property = props.globals.getNode("/consumables/fuel/tank[" ~ index ~ "]");
        var max_capacity  = tank_property.getNode("capacity-gal_us").getValue();

        var m = {
            parents: [TankEngineConsumer, EngineConsumer.new(name, engine)],
            property: tank_property,
            new_level_gal_us: nil,
        };
    ...
    test_add_fuel_flow: func (flow, dt) {
        ...
        var added_gal_us = min(me.get_max_capacity() - me.get_current_level(), consumed_gal_us);
        assert(0.0 <= added_gal_us and added_gal_us <= flow);
    ...
    prepare_add_fuel_flow: func (flow, dt) {
        me.consumed_gal_us = me.test_add_fuel_flow(flow, dt);
        me.new_level_gal_us = me.get_current_level() + me.consumed_gal_us;
        return me.consumed_gal_us;
    },
