# B707 - Lake of Constance Hangar - M.Kraus 
#
# this file is from the Tupolev 154b
...
var forceView = func{
	...
	if( n == 2 ) setprop("b707/mod-views/nav-view", 1);
	else setprop("b707/mod-views/nav-view", 0);
	# Hide right yoke
	if( n == 1 ) setprop("b707/mod-views/copilot-view", 1);
	else setprop("b707/mod-views/copilot-view", 0);

	var offset = getprop("b707/mod-views/view-offset");
	if( n > 0 ) n = n + offset;
	setprop("sim/current-view/view-number", n);
...
var modView  = func{
	...
	var mv = props.globals.getNode("b707/mod-views").getChildren("mod-view");
	if( mode == 1 )
	{
	setprop("b707/mod-views/mod", 1 );
# save current position
	setprop("b707/var/save-x", getprop("sim/current-view/x-offset-m") );
	setprop("b707/var/save-y", getprop("sim/current-view/y-offset-m") );
	setprop("b707/var/save-z", getprop("sim/current-view/z-offset-m") );
	setprop("b707/var/save-fov", getprop("sim/current-view/field-of-view") );
	setprop("b707/var/save-pitch", getprop("sim/current-view/pitch-offset-deg") );
	setprop("b707/var/save-heading",getprop("sim/current-view/heading-offset-deg"));
	setprop("b707/var/save-roll",getprop("sim/current-view/roll-offset-deg"));
# set modified view	
...
# 		getprop("sim/current-view/roll-offset-deg"));
	...
	setprop("sim/current-view/x-offset-m", getprop("b707/var/save-x") );
	setprop("sim/current-view/y-offset-m", getprop("b707/var/save-y") );
	setprop("sim/current-view/z-offset-m", getprop("b707/var/save-z") );
	setprop("sim/current-view/field-of-view", getprop("b707/var/save-fov") );
	setprop("sim/current-view/pitch-offset-deg", getprop("b707/var/save-pitch") );
	setprop("sim/current-view/heading-offset-deg",getprop("b707/var/save-heading"));
	setprop("sim/current-view/roll-offset-deg",getprop("b707/var/save-roll"));
	}
};
...
setprop("/b707/mod-views/nav-view", 0);
setprop("/b707/mod-views/copilot-view", 0);
# Do we have Model View?
if( props.globals.getNode("/sim/view[7]") != nil )
  setprop("/b707/mod-views/view-offset", 7 );
else setprop("/b707/mod-views/view-offset", 6 );
}

init_offset();

setlistener("/sim/signals/fdm-initialized", func {
view.manager.register("Engineer View", fe_view );});
