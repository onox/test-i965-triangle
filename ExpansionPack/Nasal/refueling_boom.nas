...
var get_boom_end_point = func (callback_pose) {
    ...
    x = x + getprop("/refueling/boom/origin/x-m");
    y = y + getprop("/refueling/boom/origin/y-m");
    z = z + getprop("/refueling/boom/origin/z-m");
...
var get_aar_point_mp = func (mp_node) {
    ...
    if (x == nil or y == nil or z == nil or type != "boom") {
        return nil;
    }
...
var RefuelingBoomTrackingUpdater = {
    ...
    new: func (callback_pose) {
        ...
        m.stop_automatic_movement_timer = maketimer(automatic_movement_duration, func {
            ...
            setprop("/refueling/boom/automatic-movement", 0);
        });
    ...
    is_enabled: func {
        return me.loop.timer.isRunning;
    },
    ...
    set_receiver: func (callsign) {
        ...
        setprop("/refueling/boom/contact/active", contact);
        setprop("/refueling/boom/contact/callsign", me.callsign);

        setprop("/refueling/boom/automatic-movement", contact);
        if (contact) {
    ...
    move_to: func (pose) {
        setprop("/refueling/boom/automatic-movement", 1);
    ...
    disable_boom: func {
        ...
        me.callsign = "";
        setprop("/refueling/boom/contact/active", 0);
        setprop("/refueling/boom/contact/callsign", me.callsign);
    },
    ...
    update: func (dt) {
        ...
        var distance = get_boom_end_point(me.callback_pose).direct_distance_to(fuel_point);
        ...
        if (distance <= getprop("/refueling/boom/max-contact-distance-m") + track_margin_m) {
        ...
        var origin_x = getprop("/refueling/boom/origin/x-m");
        var origin_y = getprop("/refueling/boom/origin/y-m");
        var origin_z = getprop("/refueling/boom/origin/z-m");
...
var RefuelingBoomPositionUpdater = {
    ...
    new: func (tracker) {
        ...
        m.callsign_timer = maketimer(callsign_distance_update_period, func {
            var distance = getprop("/refueling/boom/closest/distance-m");
            var callsign = getprop("/refueling/boom/closest/callsign");
            var message = sprintf("%s: %.1f meter", callsign, distance);
            setprop("/sim/multiplay/chat", message);
        });

        setlistener("/refueling/boom/closest/callsign", func (n) {
    ...
    disable: func {
        me.loop.disable();
        setprop("/refueling/boom/closest/callsign", "");
    },
    ...
    update: func (dt) {
        ...
        var end_point = get_boom_end_point(me.tracker.callback_pose);
        ...
        if (distance <= getprop("/refueling/boom/max-contact-distance-m")) {
            me.tracker.set_receiver(callsign);
        }
        elsif (distance <= getprop("/refueling/boom/max-pre-contact-distance-m")) {
            setprop("/refueling/boom/closest/distance-m", distance);
            setprop("/refueling/boom/closest/callsign", callsign);
        }
        else {
            setprop("/refueling/boom/closest/distance-m", 0.0);
            setprop("/refueling/boom/closest/callsign", "");
        }
    }
